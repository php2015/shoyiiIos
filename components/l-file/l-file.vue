	...
	methods: {
		...
		*/
		...
		wxChooseFile({url,name = 'file',header,...formData} = {}) {
			wx.chooseMessageFile({ //从聊天记录中找文件
				count: 1,
				type: 'file',
				success: ({tempFiles}) => {
		...
		*/
		...
		APP自定义保存 
		 */
		plusSaveFile(url,fileName) {
			let dtask;
			return new Promise((resolve,reject)=>{
				let downloadOptions = {
					timeout: 120,
					filename: `_downloads/files/${fileName}`,
					retryInterval: 10
				};
		 ...
		 */
		download(url,type = 'temporary',fileName) {
			if (this.loading) {
				this.toast('还有个文件玩命处理中，请稍候..');
				return;
			}
			...
			// #ifdef APP-PLUS
			if (type == 'save') {
				return this.plusSaveFile(url,fileName);
			}
			// #endif
